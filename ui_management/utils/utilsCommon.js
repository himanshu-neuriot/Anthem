...
async function newDocumentManagementDeleteFile(data,callback){
  deleteFileFromTable(data, async resultDeleteFileFromTable=> {
    if(!resultDeleteFileFromTable.error){
      deleteFileFromElasticSearch(data, async resultDeleteFileFromElasticSearch=> {
        if(!resultDeleteFileFromElasticSearch.error){
...
async function fetchAllDocuments(data,callback){
  try{
      let query='select * from contentfile where groupID = ? and deleted = ? and publishOrTrainer = ? and ingested = ?';
      let dataArray=[data.groupID,'N','P','Y'];
      mysql.querySQL({query:query,dataArray:dataArray }, async result => {
          callback(result)
      })
  }catch(err){
...
async function sendmail(user,callback){
  ...
  crypto.randomBytes(20, function (err, buf) {
    ...
    var link = config.get('verifyPasswordApi')+token;
    let tokenExpiry=Date.now() + 86400 * 1000;
    let mailoptions ={
      from: config.fromEmail,
      to : user.email,
      subject:"Welcome to Anthem search",
      html:'Dear '+user.name+',<br><br>Please click <a href="' + link + '"> here </a> to activate your account and then set password for your account.<br><br>Best Regards<br>Support Team'
    ...
    transporter.sendMail(mailoptions, function(error, info){
      ...
      if(error) {
        ...
        response={error:true,result:{status:400,send:{"status":false,res:"Email Id is invalid or some error occured"}}}
        callback(response)
      }
      else{
...
async function contactsSendMail(user,callback){
  ...
  let mailoptions ={
    ...
    to : user.email,
    subject:"Welcome to Anthem search",
    html:'Dear '+user.name+',<br><br>We have received your inquiry about '+user.purpose+'. Thank you for contacting us. We will be in touch with you shorty.<br><br>Best Regards<br>Support Team'
  };
  transporter.sendMail(mailoptions, function(error, info){
      var response;
      if(error) {
...
async function forgetPasswordSendmail(user,callback){
  let transporter=nodemailer.createTransport({
    service: 'Godaddy',
    host: "smtpout.secureserver.net",  
    secureConnection: true,
  ...
  let mailoptions ={
    ...
    to : user.userID,
    subject:"Welcome to Anthem search",
    html:'Dear '+user.name+',<br><Br>Please click <a href="' + user.link + '"> here </a> to reset password for your account.<br><br>Best Regards<br>Support Team'
  }
  ...
  transporter.sendMail(mailoptions, function(error, info){
    ...
    if(error) {
      console.log("error in sending mail",error);
     response={error:true,result:'Some Error Occurred'}
    }
    else{
      response={error:false,result:'Mail to reset password is successfully sent on entered email ID'}
        }
        callback(response)
    });
 ...
 module.exports={
newDocumentManagementDeleteFile,
    sendmail,
    contactsSendMail,
